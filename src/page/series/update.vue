...
<template>
  <div class="container">
    <div class="top">
      <h3>系列课&gt;<span class="blue">编辑</span></h3>
    ...
    <div class="content">
      <Form :model="formValidate" :label-width="180" class="form" ref="form" :rules="ruleValidate">
        ...
        <FormItem label="收费类型" prop="pay_type">
          <Select v-model="formValidate.pay_type" disabled>
  ...
  export default {
    ...
    data() {
      return {
        ...
        courseIntro: [],
        formValidate: {
          id: this.$route.params.id, // 系列课的id
          name: '', // 课程标题
          pay_type: 0, // 收费类型
    ...
    created() {
      ...
      this.getSeriesDetails(this.formValidate.id)
    },
    computed: {
      sharePrice() {
      ...
      getSeriesDetails(id) {
        api.getSeriesDetails({id: id}).then((res) => {
          if (res) {
            ...
            this.formValidate.plan_period_count = res.plan_period_count

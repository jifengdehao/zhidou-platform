...
<template>
  <div class="container">
    <div class="top">
      <h3>系列课时&nbsp;&gt;&nbsp;<span class="blue">编辑</span></h3>
    ...
    <div class="content">
      <Form :label-width="180" class="form" ref="formValidate" :model="formValidate" :rules="ruleInline">
        ...
        <FormItem label="课程形式">
          <Select v-model="formValidate.period_type" disabled>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取系列课时详情
      getPeriodDetails() {
        api.getSeriesPeriodDetails({id: this.formValidate.id}).then((res) => {
          if (res) {
            console.log(res)
            this.formValidate.subject = res.title
